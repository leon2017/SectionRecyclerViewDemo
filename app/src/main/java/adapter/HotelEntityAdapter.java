...
public class HotelEntityAdapter extends SectionedRecyclerViewAdapter<HeaderHolder, HotelEntityAdapter.DescHolder, RecyclerView.ViewHolder> {


    public ArrayList<HotelEntity.TagsEntity> allTagList;
    private Context mContext;
    private LayoutInflater mInflater;
    private final TreeSet<String> selectedSet;

    private SparseBooleanArray mBooleanMap;

    public HotelEntityAdapter(Context context, TreeSet<String> selectedSet) {
        mContext = context;
    ...
    protected void onBindItemViewHolder(DescHolder holder, int section, int position) {
        holder.mTagInfo = allTagList.get(section).tagInfoList.get(position);
        holder.descView.setText(allTagList.get(section).tagInfoList.get(position).tagName);
        holder.descView.setSelected(allTagList.get(section).tagInfoList.get(position).isSelected);
    }

    public class DescHolder extends RecyclerView.ViewHolder {

        TextView descView;
        HotelEntity.TagsEntity.TagInfo mTagInfo;

        DescHolder(final View itemView) {
            super(itemView);
            ...
            descView.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    if (mTagInfo.isSelected) {
                        selectedSet.add(mTagInfo.tagName);
                    } else {
                        selectedSet.remove(mTagInfo.tagName);
                    }
                }
            });
        }
